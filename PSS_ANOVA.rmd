...
set.seed(81)
group1 <- rnorm(10, mean = 3, sd = 1.3)
group2 <- rnorm(10, mean = 1, sd = 1.3)
df <- tibble(group1, group2) %>% pivot_longer(cols = c("group1","group2"),
                                              names_to="group",
                                              values_to="response")

ggplot(data = df, aes(x = group, y = response)) + 
    geom_boxplot(aes(color = group),width = 0.2, show.legend = F) +
  ...
  theme_bw() + 
  labs(
    title = "Mouse biomarker level after treatment",
    x = "Treatment Group",
    y = "Response (ug/mL)"
  )
```
...
df2sample <- 2*length(group1)-2
lambda <- (mean(group1)-mean(group2))/sqrt(sd(group1)^2/length(group1)+sd(group2)^2/length(group2))
tstar <- qt(0.975, df=df2sample)

#graph it
#prettier plot with double shading
twosample.data <- tibble(xvar = seq(0, 7, by = 0.1)) %>%
  mutate(
    #distribution under the null hypothesis
         H0 = dt(xvar, df = df2sample),
  #distribution under the alternative hypothesis
         HA = dt(xvar, df = df2sample, ncp = lambda)) %>%
  pivot_longer(cols = c(H0, HA),
               names_to = "hypothesis",
               values_to = "pdf")

pwr.t.test(n = 10, d=2/1.3, sig.level = 0.05)

plota <- ggplot(twosample.data, aes(x = xvar, y = pdf,
                    color = hypothesis,
                    linetype = hypothesis)) +
  geom_line() + 
  geom_area(data = twosample.data %>% 
              filter(hypothesis == "HA"),
            aes(y=ifelse(xvar > tstar, pdf, 0)),
                fill = "lightyellow") +
  geom_area(data = twosample.data %>%
  ...
  labs(title = "Power = 0.90",
       subtitle = "n = 10, delta = 2, sd = 1.3; lambda = 3.79",
       x = "Value of t statistic")

#example 2: delta small compared to sd
group3 <- rnorm(10, mean = 2, sd = 1.3)
group4 <- rnorm(10, mean = 1, sd = 1.3)
df2sample2 <- 2*length(group3)-2
lambda2 <- (mean(group3)-mean(group4))/sqrt(sd(group3)^2/length(group3)+sd(group4)^2/length(group4))

pwr.t.test(n = 10, d=1/1.3, sig.level = 0.05)

twosample.data2 <- tibble(xvar = seq(0, 7, by = 0.1)) %>%
  mutate(
    #distribution under the null hypothesis
         H0 = dt(xvar, df = df2sample),
  #distribution under the alternative hypothesis
         HA = dt(xvar, df = df2sample, ncp = lambda2)) %>%
  pivot_longer(cols = c(H0, HA),
               names_to = "hypothesis",
               values_to = "pdf")

plotb <- ggplot(twosample.data2, aes(x = xvar, y = pdf,
                    color = hypothesis,
                    linetype = hypothesis)) +
  geom_line() + 
  geom_area(data = twosample.data2 %>% 
              filter(hypothesis == "HA"),
            aes(y=ifelse(xvar > tstar, pdf, 0)),
                fill = "lightyellow") +
  geom_area(data = twosample.data2 %>%
  ...
  theme(legend.position = "none") +
  scale_color_viridis_d() +
  labs(title = "Power = 0.37",
       subtitle = "n = 10, delta = 1, sd = 1.3; lambda = 1.40",
       x = "Value of t statistic")
...
```{r echo = F, fig.dim=c(8,6), fig.cap = "Comparing the means of 3 groups"}
#create simulated data for example
set.seed(81)
treat.a <- rnorm(10, mean = 5.5, sd = 1.3)
treat.b <- rnorm(10, mean = 2.5, sd = 1.5)
placebo <- rnorm(10, mean = 4, sd = 1.4)

#put simulated data in nice format for graphing
exp.1 <- tibble(treat.a, treat.b, placebo)
exp.1long <- pivot_longer(exp.1, 
                          cols = c("treat.a",
                                   "treat.b",
                                   "placebo"),
                          names_to = "treatment",
                          values_to = "response")
exp.1long$treatment <- factor(exp.1long$treatment)
levels(exp.1long$treatment) <- c("Placebo",
                                 "Treatment A",
                                 "Treatment B")

plot1 <- ggplot(exp.1long, aes(x=treatment, y = response, fill = treatment)) +
  geom_violin(width = 0.4, alpha = 0.4, show.legend = F) +
...
#plot showing the grand mean, group means and individual data points
plot2 <- ggplot(data = exp.1long, aes(x= treatment, y = response, fill = treatment)) +
  geom_dotplot(binaxis = "y", alpha = 0.5) +
  geom_hline(aes(yintercept = mean(response)),
             lty = "dashed") +
  stat_summary(fun.y = "mean", geom = "point", size = 4, color = "grey33", alpha = 1) + 
  theme_bw() + 
  theme(legend.position = "none")
...
```{r echo=F, fig.cap = "Power for different effect sizes as a function of n per group"}
#tidy(pwr.anova.test(f = 1, k = 3, n=3, sig.level = 0.05) )

#we want to plot power for a number of different sizes of n
cohensf <- c(0.1, 0.25, 0.5, 1.0, 1.5)
...
ggplot(powerdata, aes(x = n, y = power, color = factor(cohensf))) + 
  ...
  theme_minimal() + 
  labs(
    title = "Power of a one-way ANOVA with 3 balanced groups",
