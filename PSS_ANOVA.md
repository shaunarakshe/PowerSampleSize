...
**We can estimate a minimum value for $\sigma^2_m$ from a difference in means.**

You probably have an idea what a meaningful difference in means would be for your experiment!

## Effect size: a better way
...
The problem with the power calculations above is that we had to specify the mean of each level.  Sometimes what we want is a way to compute power or sample size based on a meaningful difference between level means!
